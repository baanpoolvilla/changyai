-- =====================================================
-- BaanPool Ops — LINE notify ALL roles except CEO on work order created
-- =====================================================
-- Simplify: when a work order is inserted (or assigned_to changes),
--   1. Assigned person → Flex card "คุณได้รับมอบหมายงานใหม่"
--   2. Everyone else with line_user_id AND role != 'owner' → text message
-- CEO (role = 'owner') receives NO LINE notification.
-- =====================================================
...
BEGIN
  ...
  -- ─── common text message (for non-assigned recipients) ───────────────
  v_text_msg :=
    '📝 ใบงานใหม่' || chr(10)
    || '📋 ' || NEW.title || chr(10)
  ...
  -- ─── 1. Notify assigned_to → Flex card ───────────────────────────────
  ...
  -- ─── 2. Everyone else: role != 'owner', has line_user_id, not yet sent ─
  FOR v_recipient IN
