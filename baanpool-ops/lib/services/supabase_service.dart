...
class SupabaseService {
  ...
  Future<List<Map<String, dynamic>>> getPurchaseOrders({
    ...
    var query = _client.from('purchase_orders').select('*, creator:created_by(full_name)');
    if (status != null) query = query.eq('status', status);
    if (propertyId != null) query = query.eq('property_id', propertyId);
  ...
  Future<Map<String, dynamic>> getPurchaseOrder(String id) async {
    return await _client.from('purchase_orders').select('*, creator:created_by(full_name)').eq('id', id).single();
  }
