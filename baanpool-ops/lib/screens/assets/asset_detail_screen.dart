...
class _AssetDetailScreenState extends State<AssetDetailScreen> {
  ...
  /// รายชื่อคนที่มอบหมาย PM ได้ — ใช้ร่วมกันทั้งตอนเพิ่มและตอนแก้ไข
  /// ถ้ายังไม่มีใครเป็นช่างเลย ให้เลือกจากผู้ใช้ทั้งหมดแทน
  Future<List<Map<String, dynamic>>> _loadAssignableUsers() async {
    List<Map<String, dynamic>> users = [];
    try {
      users = await _service.getTechnicians();
    } catch (_) {}
    if (users.isEmpty) {
      try {
  ...
  Future<void> _showAddScheduleDialog() async {
    ...
    final technicians = await _loadAssignableUsers();
