...
enum PmFrequency {
  ...
  /// จำนวนรอบต่อปีสูงสุดที่เป็นไปได้ = จำนวนช่องที่ยังตกอยู่ในปีเดียวกัน
  /// นับจากวันตั้งต้น เช่น ทุก 3 เดือน → เดือนที่ 0,3,6,9 = 4 รอบ
  ///                    ทุก 7 เดือน → เดือนที่ 0,7 = 2 รอบ (ไม่ใช่ 1)
  /// (ความถี่แบบสัปดาห์ไม่ใช้ระบบรอบต่อปี จึงคืน 0)
  int get maxRoundsPerYear {
    final m = months;
    if (m == null) return 0;
    return (12 + m - 1) ~/ m; // ปัดขึ้น
  }

  /// ตัวเลือก "ทำกี่รอบต่อปี" ที่ให้ผลต่างจากแบบทำต่อเนื่องจริงๆ
  ///
  /// ถ้า 12 หารด้วยความถี่ลงตัว (เช่น ทุก 3 เดือน → 4 รอบ) การทำครบทุกช่อง
  /// คือแบบต่อเนื่องอยู่แล้ว จึงตัดตัวเลือกสุดท้ายออกไม่ให้ซ้ำซ้อน
  /// แต่ถ้าหารไม่ลงตัว (เช่น ทุก 7 เดือน → 25/2, 25/9 แล้ววนกลับ 25/2 ปีหน้า)
  /// รอบสุดท้ายยังต่างจากต่อเนื่อง จึงต้องเลือกได้
  List<int> get roundsPerYearOptions {
    final m = months;
    if (m == null || m >= 12) return const [];
    final last = 12 % m == 0 ? maxRoundsPerYear - 1 : maxRoundsPerYear;
